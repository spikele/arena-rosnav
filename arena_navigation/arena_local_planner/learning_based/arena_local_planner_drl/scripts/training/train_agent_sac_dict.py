...
def main():
    ...
    if not args.debug and ns_for_nodes:
        env = SubprocVecEnv(
            [make_envs_dict(args, ns_for_nodes, i, params=params, PATHS=PATHS) for i in range(args.n_envs)],
            start_method="fork",
        )
    else:
        env = DummyVecEnv([make_envs_dict(args, ns_for_nodes, i, params=params, PATHS=PATHS) for i in range(args.n_envs)])
