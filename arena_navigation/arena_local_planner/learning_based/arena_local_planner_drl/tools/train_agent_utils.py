...
def initialize_hyperparameters_thesis(PATHS: dict, load_target: str, config_name: str = "default", n_envs: int = 1) -> dict:
    """
    ...
    if load_target is None:
        hyperparams = load_hyperparameters_json_thesis(PATHS=PATHS,  from_scratch=True, config_name=config_name)
        hyperparams["robot"] = rospy.get_param("model", "not specified")
        hyperparams["agent_name"] = PATHS["model"].split("/")[-1]
    else:
        hyperparams = load_hyperparameters_json_thesis(PATHS=PATHS)
...
def load_hyperparameters_json_thesis(PATHS: dict, from_scratch: bool = False, config_name: str = "default") -> dict:
    """
    ...
    if os.path.isfile(doc_location):
        ...
        check_hyperparam_format_thesis(loaded_hyperparams=hyperparams, PATHS=PATHS)
        return hyperparams
    else:
        if from_scratch:
...
def check_hyperparam_format_thesis(loaded_hyperparams: dict, PATHS: dict) -> None:
...
def update_hyperparam_model_thesis(model: PPO, PATHS: dict, params: dict, n_envs: int = 1) -> None:
    """
