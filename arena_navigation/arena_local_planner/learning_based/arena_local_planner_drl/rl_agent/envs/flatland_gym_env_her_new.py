    ...
    '''def compute_reward(self, achieved_goal, desired_goal, info):
        ...
        if length > 2:
            ...
            for i in range(length):
                distance = (achieved_goal[i][0] ** 2 + desired_goal[i][0] ** 2 - 2*achieved_goal[i][0]*desired_goal[i][0]*np.cos(achieved_goal[i][0]-desired_goal[i][1])) ** 0.5
                reward[i] = 0 if distance < self.goal_radius else -1
            return reward
        
        else:
            distance = (achieved_goal[0] ** 2 + desired_goal[0] ** 2 - 2*achieved_goal[0]*desired_goal[0]*np.cos(achieved_goal[0]-desired_goal[1])) ** 0.5
            return 0 if distance < self.goal_radius else -1
