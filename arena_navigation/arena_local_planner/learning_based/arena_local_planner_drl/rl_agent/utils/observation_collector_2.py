...
class ObservationCollector:
    def __init__(
        ...
        #obsshape = ", ".join(self.observation_space.shape)
        print("Observation space shape: " + str(self.observation_space.shape))
    ...
    def get_observations(self, *args, **kwargs):
        ...
        if self._is_train_mode:
            ...
            try:
                ...
                # with action publisher:
                rospy.wait_for_message(f"{self.ns_prefix}next_cycle", Bool)
                # without action publisher:
                #rospy.wait_for_message(f"/cmd_vel")
                #print("after waiting for message!")
            except Exception:
                pass
    ...
    def process_cmd_vel_msg(self, msg_cmd_vel):
        ...
        ang_z = msg_cmd_vel.angular.z
        return np.array([lin_x, lin_y, ang_z])
